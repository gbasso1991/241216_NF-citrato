...
fig2, ax = plt.subplots(2, 1, figsize=(9,4), constrained_layout=True,sharex=True)
...
ax[1].plot(np.arange(len(Mag_max)), Mag_max,'v-',label='M$_{max}$')
ax[1].axhline(xi_patron_Dy2O3_v*Hmax + max(Ciclo_descancelacion_M_filt),0,1,c='k',ls='--',label='M$_{max}$ '+nombre[1:]+'\n+ descancelacion')


ax[1].set_ylabel('Mag Máxima')
...
plt.savefig(os.path.join(output_dir,os.path.commonprefix(list(fnames_m))+'_Hc_Mag_max_vs_indx.png'),dpi=300,facecolor='w')
